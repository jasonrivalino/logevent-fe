...
export function CommandLeft() {
    ...
    return (
        <div className="bg-white border-2 rounded-xl w-full md:w-80 max-h-[31rem] mb-4 md:mb-0 mt-[0.1rem]">
            ...
            <div className="px-6 pb-6 text-black font-sofia">
                <div>
                    <button 
                        className={`flex items-center mb-2 text-lg p-2 w-full rounded-lg ${pathname === '/admin/statistics' ? 'border-pink-400 border-2 shadow-lg' : 'bg-white'}`}
                        onClick={() => router.push('/admin/statistics')}
                    >
                        <FaChartBar className="ml-2 mr-5 text-pink-900" /> Statistik
                    </button>
                    <button 
                        className={`flex items-center mb-2 text-lg p-2 w-full rounded-lg ${pathname === '/admin/order-recap' ? 'border-pink-400 border-2 shadow-lg' : 'bg-white'}`}
                        onClick={() => router.push('/admin/order-recap')}
                    >
                        <FaClipboardList className="ml-2 mr-5 text-pink-900" /> Rekap Pesanan
                    </button>
                    <button 
                        className={`flex items-center mb-2 text-lg p-2 w-full rounded-lg ${/^\/admin\/manage-vendor/.test(pathname) ? 'border-pink-400 border-2 shadow-lg' : 'bg-white'}`}
                        onClick={() => router.push('/admin/manage-vendor')}
                    >
                        <FaUserPlus className="ml-2 mr-5 text-pink-900" /> Kelola Vendor
                    </button>
                    <button 
                        className={`flex items-center mb-2 text-lg p-2 w-full rounded-lg ${/^\/admin\/manage-event-package/.test(pathname) ? 'border-pink-400 border-2 shadow-lg' : 'bg-white'}`}
                        onClick={() => router.push('/admin/manage-event-package')}
                    >
                        <FaCalendarAlt className="ml-2 mr-5 text-pink-900" /> Kelola Paket Event
                    </button>
                    <button 
                        className={`flex items-center mb-2 text-lg p-2 w-full rounded-lg ${pathname === '/admin/manage-faq' ? 'border-pink-400 border-2 shadow-lg' : 'bg-white'}`}
                    ...
                    <button
                        className={`flex items-center text-lg p-2 w-full rounded-lg ${pathname === '/admin/settings' ? 'border-pink-400 border-2 shadow-lg' : 'bg-white'}`}
                        onClick={() => router.push('/admin/settings')}
                    >
                        {/* Import settings icon from react icons */}
                        <FaGear className="ml-2 mr-5 text-pink-900" /> Pengaturan
                    </button>
                </div>
            </div>
        </div>
